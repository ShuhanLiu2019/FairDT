...
def fair_metric(val_labels,sens_test,val_preds): 
    ...
    idx_s0_y0 = (idx_s0 & (val_y == 0)).to(torch.bool)
    idx_s1_y0 = (idx_s1 & (val_y == 0)).to(torch.bool)
    ...
    predic  =  abs(sum(pred_y[idx_s0_y0])/sum(idx_s0_y0)-sum(pred_y[idx_s1_y0])/sum(idx_s1_y0))
    eodds = equality+predic

    return parity,equality,predic,eodds
...
if __name__ == "__main__":
    ...
    for epoch in range(1000):
        ...
        with th.no_grad():  
            ...
            parity,equality,predic,eodds = fair_metric(label_test,sens_test,test_preds)
        ...
        if acc_test > args.acc and f1_test > args.f1:
            if best_fair > parity + equality:
                ...
                best_result['predic'] = predic
                best_result['eodds'] = eodds
    ...
    if len(best_result) > 0:
        ...
        log_txt +=  "accuracy:{:.4f}, f1:{:.4f}, parity:{:.4f}, equality:{:.4f}, predic:{:.4f}, eodds:{:.4f}\n"\
                    .format(best_result['acc'],best_result['f1'], best_result['parity'],best_result['equality'],best_result['predic'],best_result['eodds'])

        with open(f'log_addmetric.txt', 'a') as f:
            f.write(log_txt)

        log_csv =   f"[{current_time}], {args.dataname}, {a}, {b}, {phi},"
        log_csv +=  "{:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}\n"\
                    .format(best_result['acc'],best_result['f1'], best_result['parity'],best_result['equality'],best_result['predic'],best_result['eodds'])

        with open(f'log_addmetric.csv', 'a') as f:
            f.write(log_csv)
        ...
        print("Test:",
            ...
            "predic: {:.4f}".format(best_result['predic']),
            "eodds: {:.4f}".format(best_result['eodds']))
